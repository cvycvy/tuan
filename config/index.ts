...
/**
 * 后端 API 基地址（如 https://hongbei.example.com，不带结尾斜杠）。
 * - H5：留空即可，走同源相对路径 /api，由 Nginx 反代
 * - 小程序：必填，wx.request 不支持相对路径
 */
...
/** 写入微信小程序 project.config.json，注入真实 AppID 与产物目录 */
const generateWeappProjectConfig = (outputRoot: string) => {
  const appid = (process.env.TARO_APP_WEAPP_APPID || '').trim();
  if (!appid) {
    return;
  }
  ...
  // 以根目录 project.config.json 为基底，仅覆盖关键字段，保留开发者工具的其他设置
  let config: Record<string, any> = {
    miniprogramRoot: './',
    compileType: 'miniprogram',
    setting: { urlCheck: true, es6: false, postcss: false, minified: false },
  };
  const rootConfigPath = path.resolve(__dirname, '..', 'project.config.json');
...
export default defineConfig<'vite'>(async (merge, _env) => {
  ...
  // 小程序端必须配置后端绝对域名，否则 wx.request 会因相对路径直接失败
  if (isWeapp && !apiBase) {
    throw new Error(
      '[构建失败] 微信小程序缺少后端域名配置：请在 .env.local 设置 ' +
        'TARO_APP_API_BASE=https://你的HTTPS域名（域名需在微信公众平台「request 合法域名」中配置）',
    );
  }
  ...
  const baseConfig: UserConfigExport<'vite'> = {
    ...
    defineConstants: {
      PROJECT_DOMAIN: JSON.stringify(apiBase),
